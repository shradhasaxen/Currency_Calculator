...
function insertCharacter(char) {
  ...
  function clearDisplay() {
    ...
    inputBar.classList.add('clear-anim'); // Add the animation class to trigger the animation
    inputBar.value = ''; // Clear the input bar
    
    // Remove the animation class after the animation is complete
    inputBar.addEventListener('animationend', function() {
      inputBar.classList.remove('clear-anim');
    });
  }
...
function calculate() {
  ...
  function showHistory() {
    const display = document.getElementById('inputBox');
    display.value = history.join('\n'); // Join the history array with new lines and display it in the input box
  }
